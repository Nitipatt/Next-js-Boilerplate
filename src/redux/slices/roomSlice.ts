...
import type { Room } from './roomSlice.d';
...
export const fetchRooms = createAsyncThunk('rooms/fetchRooms', async () => {
  ...
  const response = await fetch('https://pantip.com/api/forum-service/home/get_room_recommend?tracking_code=%7Bsgo5dj1833zExjWaHczth%7D', {
    method: 'GET',
    headers,
  });
...
const roomSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(fetchRooms.fulfilled, (_, action) => {
      return action.payload;
    });
  },
});
